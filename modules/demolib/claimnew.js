...
var classify = require("./classify-claim-with-watson.js");

function claim01r() {
  pjs.defineDisplay("display", "claimnewd.json", { rrnFields: { claimsfl: 'rrn' } });

  pjs.defineTable("claimsp", {  read: true,  update: true,  write: true,  delete: true,  keyed: true});
  pjs.defineTable("claims2l", {  rename: { CLAIMS: 'CLAIMS2' },  read: true,  keyed: true});
  ...
  function loadDetails() {
    ...
    cmmdy = pjs.dec(cmdate, "*mdy");
    mapaddress = `${cmaddr1} ${cmcity} ${cmstate} ${cmzip}`
  ...
  function showDetails() {
    ...
    do {
      ...
      if (uploadinfo === '001') {  // 1 file uploaded
        ...
        var obj = classify(imagefile);
         
          if (obj && obj.score > 0.75) {
          let newClass = null
          if (obj.class == "motorcycleaccident") { cmmotor = "Y"; newClass = "Motorcylce Involved"; }
          if (obj.class == "flattire") { cmflattire = "Y";newClass = "Flat Tire"; }
          if (obj.class == "brokenwinshield") { cmbrokenw = "Y"; newClass = "Broken Windshield"; }
          if (obj.class == "vandalism") { cmvandal = "Y"; newClass = "Vandalism"; }
          if (obj.class == "pedestrian") { cmpedestr = "Y"; newClass = "Pedestrian Involved"; }
          if(newClass){
            pjs.messageBox(`Classification changed to ${newClass} based on image analysis.`)
          }
        }
        // new code end
